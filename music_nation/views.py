...
# from machine_learning.predictions import pm_predict, is_model_predict
...
#     return render(request, 'music_nation/home_song.html',{'Songs':Songs})
...
# def song_detail(request, username):
#     # show all albums of the artist
#     songs = get_object_or_404(User, username=username)
#     songs = Song.albums.all()
#     return render(request, 'music_nation/song_detail.html', {'albums':songs, 'username':username})
...
def album_detail(request,username, album):
    ...
    # recommendation_purpose = list(songs) # 

    # popular_recommendations = pm_predict(recommendation_purpose)
    # personalized_recommendations = is_model_predict(recommendation_purpose)
    
    return render(request, 'music_nation/album_detail.html', {'songs':songs, 'album':album, 'username':username
    }) # i have predicted vars for predicted recommendations
